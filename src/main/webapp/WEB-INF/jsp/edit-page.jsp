...
<div class="registration-form">
    <form id="editPetForm" enctype="multipart/form-data">
        ...
        <% String id = (String) request.getAttribute("pet"); %>
        <div class="form-group">
            <button type="button" class="btn btn-block create-account" onclick="editForm('<%= id %>')">edit pet</button>
        </div>
    </form>
</div>
<script>
    async function editForm(id) {
        ...
        const response = await fetch('http://localhost:8080/api/edit-pet/'+id, {
            method: 'PUT',
            body: formData,
        });
        ...
        try {
            if (response.ok) {
                alert('Pet editing successfully!');
            }else {
                throw new Error('problem editing while editing pet');
            }
        }catch(error) {
